...
const getAllAssigneedIsssueSvc = async (userEmail) => {
    try {
...
const getAllReportedIssueSvc = async (userEmail) => {
    try {
        const allUserIssue = await Issue.find({ 'reporter.email': userEmail })
        if (!allUserIssue) {
            const error = createHttpError('No Issue Found for the User', 400)
            throw error
        }
